...
| Password policy | Minimum length **12**, matching `MIN_PASSWORD_LENGTH` — must not be *lowered* to meet the client. The **Password requirements** dropdown left unset (it reads "Select an option"), deliberately; see below. Set 2026-09-12 | Supabase → Authentication → Sign In / Providers → Email |
| Secure password change | **on** — a session older than 24 hours cannot change a password without an emailed one-time code, which is how a stolen session is stopped from locking the owner out. Sessions created by a recovery link are exempt in Supabase's own code, so the forgot-password flow is unaffected; `errReauthNeeded` explains the refusal to anyone else. Its weaker sibling "Require current password when updating" is inert while every account is OAuth-only — Supabase applies it only to accounts that already have a password. Confirmed 2026-09-12 | Supabase → Authentication → Sign In / Providers → Email |
| Anonymous sign-ins | **disabled**. Two other things now refuse one regardless: `verify_jwt` rejects a token claiming `is_anonymous`, and the mirroring trigger cannot write a `public.users` row without an email address. Only the first of those is a decision rather than an accident. Confirmed 2026-09-12 | Supabase → Authentication → Sign In / Providers |
...
| Setting | Expected | Where |
| --- | --- | --- |
| Sentry issue alert rules | One per project, both **enabled**, both named "Send a notification for high priority issues" — `splitdec-api` #797498, `splitdec-frontend` #797497. Conditions: a new high-priority issue, or an existing one escalating. These are Sentry's **auto-created defaults** from project creation (2026-09-01, seven seconds apart); nobody designed them, they merely happen to fit. No metric alert rules exist, so there is per-issue alerting and no rate-spike alerting. Confirmed 2026-09-17 | Sentry → `splitdec-api` / `splitdec-frontend` → Alerts |
| Sentry notification target | **Email only**, to `issue_owners` with fallthrough `ActiveMembers`. No ownership rules are configured, so every alert falls through to the org's active members — one account, the maintainer's. No Slack, no PagerDuty, no webhook: if that mailbox is not read, the alert is not received. Confirmed 2026-09-17 | Sentry → Settings → Notifications |
| Does an ERROR actually page? | **Yes, proven once.** `splitdec-api` rule #797498 last triggered 2026-09-08T10:11:50Z, thirteen seconds after `SPLITDEC-API-2` (the `/api/health/sentry` probe, `level: error`) was first seen. So error-level → high priority → rule fires → email. **The one link still unproven** is that a `logentry` event — what `LoggingIntegration` makes of `logger.error`, and therefore what every alert added in September 2026 actually is — is classified the same as the `message` event that probe produced. Both carry `level: error` and Sentry's priority is level-derived, so it should be; nothing has demonstrated it. The cheap test is to unset `RESEND_API_KEY` on a preview deployment and send one invitation | Sentry → Alerts → rule history |
