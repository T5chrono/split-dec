...
| Artifact hashes, both ecosystems | `package-lock.json` (`sha512-` on every entry); `requirements.txt` / `requirements-dev.txt` (`--generate-hashes`, SHA-256 per artifact, added 2026-09-15). A pin names a version; a hash names the file, so a substituted mirror is refused rather than installed. pip switches hash checking on by itself; Vercel builds with uv, which does not, so `vercel.json` sets `build.env.UV_REQUIRE_HASHES`. Both halves guarded by `tests/test_requirements_locks.py` and `tests/test_vercel_config.py`, inside the required `backend` check — hashes are all-or-nothing, so losing them stops an install failing and starts it checking nothing |
